...
proc links(tag: string, con: AsyncRedis): Future[VNode] {.async.} =
    ...
    let articles: seq[Article] = (await all(keys.map(proc (x: auto): auto = x.read(con))))
        ...
        .filter(proc (x: auto): auto = x.tags.contains(tag))

    let content = build_html(tdiv(class="has-side-note")):
        h1: text fmt"Articles with tag {tag}"
        verbatim intro_text
...
proc tag_articles*(ctx: Context) {.async.} =
    let tag = ctx.get_path_params_option("tag").get();
    let base_elem = content_elem()
    let con = await read_con()
    let tags = await tags(con)
    let links = await links(tag, con)
    ...
    let text = base(fmt"Articles | {tag}", "/articles", content = base_elem)
    await con.quit()
    resp $text
